...
Proxlane は、NAT やルーターのポート開放をせずに、ローカルマシン上の開発サーバーや TCP サービスを外部公開できるトンネルサービスである。

Proxlane は OSS として開発する。ソースコード、Issue、Pull Request、デプロイ例が公開される前提で、秘匿情報をリポジトリに含めない設計と運用を必須とする。
...
```text
https://blue-sky-123.t.proxlane.com -> http://localhost:3000
tcp://jp-1.tcp.proxlane.com:21432   -> localhost:22
```
...
| 用語 | 意味 |
| --- | --- |
| Agent | ユーザーのローカルマシンで動く `proxlane` CLI/常駐プロセス |
| Relay / Edge | グローバル IP を持ち、外部通信を受ける Proxlane サーバー |
...
Proxlane 側が Relay / Control Plane / Dashboard を運用する。ユーザーは GitHub OAuth でログイン後、Agent をインストールして使う。
...
| DB-003 | schema と migration は `apps/web/prisma/schema.prisma` と Prisma Migrate で管理する | P1 |
...
| CLI-002 | `proxlane auth <TOKEN>` でローカル設定にトークンを保存できる | P1 |
| CLI-003 | `proxlane http <PORT>` で HTTP トンネルを開始できる | P1 |
| CLI-004 | `proxlane tcp <PORT>` で TCP トンネルを開始できる | P1 |
...
| CLI-009 | `proxlane update` で自己更新できる | P3 |
...
| BILL-007 | MVP の Hosted SaaS は Free / Plus / Pro の 3 区分で設計する | P2 |
| BILL-008 | Free は 0円、Plus は 390円/月、Pro は 980円/月とする | P2 |
...
| ID | 要件 | 優先度 |
| --- | --- | --- |
| OSS-001 | Proxlane は OSS として公開開発する | P1 |
...
- CLI は v0.1 で Windows / Linux に対応し、macOS は後続で追加する。
- Go の single binary として配布する。
- 初期配布は GitHub Releases と install script を正とし、Scoop / apt / Homebrew は後続で検討する。

## 8. MVP 範囲

最初の MVP は「Proxlane Core」として、次だけを作る。
...
- `proxlane auth <TOKEN>` で token が保存される。
- `proxlane http 3000` を実行すると `https://<random>.t.proxlane.com` が表示される。
- 外部ブラウザからその URL にアクセスすると `localhost:3000` に到達する。
- WebSocket echo サーバーがトンネル越しに動く。
- `proxlane tcp 22` を実行すると公開 TCP アドレスが表示される。
...
2026-05-17 時点で、ngrok の公開ドキュメントでは agent の authtoken 認証、HTTP/S endpoint、TCP endpoint、ランダム URL、固定 TCP address、独自ドメイン、Traffic Policy などが説明されている。Proxlane はこれらを参考にしつつ、初期実装では HTTP/TCP トンネルに絞る。
