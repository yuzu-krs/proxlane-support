...
sequenceDiagram
    ...
    User->>Agent: proxlane auth <TOKEN>
...
- tunnel 作成後に local HTTP/TCP service の疎通を自動確認し、その後も状態変化時だけ OK/WARN を表示する。失敗時も tunnel は維持する。
- `doctor` で config、token、Relay、DNS、local HTTP/TCP service の問題を切り分ける。
...
proxlane start --config proxlane.yml
proxlane status
```
...
- HTTP tunnel の Basic Auth / Bearer token protection を Relay 上で判定する。
...
- Relay からの内部 API request を `PROXLANE_RELAY_INTERNAL_SECRET` で認証する。
...
| Plus | 390円 | 3 active tunnel、TCP/HTTP 各3、予約 subdomain 1、50GB/月 |
| Pro | 980円 | 10 active tunnel、TCP/HTTP 各10、予約 subdomain 5、固定 TCP port は後で実装、200GB/月 |

MVP では Stripe の usage-based billing は使わず、Proxlane 側の利用量集計で quota を制御する。Stripe には subscription status と price id の同期だけを求める。
...
| 4610 | Agent control | MVP の制御接続。Hosted では `connect.proxlane.com:443` も検討 |
| 10000-20000 | TCP endpoint | MVP の公開 TCP port range。自動割り当ては範囲内のランダム開始位置から探索する |
| 4611 | Admin API | private network / localhost 推奨 |

初期環境では、これらを開発者所有のグローバル IP 付き Linux VPS 1 台に集約する。VPS firewall では必要な port のみ開放し、SSH は鍵認証のみ、root login 無効、systemd または Docker で Relay を常駐させる。
...
```text
proxlane.com              A/AAAA -> Dashboard / public home
connect.proxlane.com      A/AAAA -> Relay control
*.t.proxlane.com          A/AAAA -> HTTP tunnel Relay
jp-1.tcp.proxlane.com     A/AAAA -> TCP Relay
```

HTTP tunnel は wildcard DNS を使って `https://<name>.t.proxlane.com` へ集約する。Dashboard/Auth.js の cookie は host-only にし、`.proxlane.com` domain cookie を発行しない。ユーザーが公開する任意コンテンツを `*.t.proxlane.com` に分離し、Dashboard と tunnel host の cookie 境界を保つ。
...
```json
{
  ...
  "public_url": "https://blue-sky-123.t.proxlane.com"
}
```

### 6.2 Routing

1. Relay が `https://blue-sky-123.t.proxlane.com` への request を受ける。
...
X-Proxlane-Tunnel-Id: <tunnel-id>
```
...
```json
{
  ...
  "public_url": "tcp://jp-1.tcp.proxlane.com:21432",
  "remote_port": 21432
}
```
...
```text
~/.proxlane/config.yaml
```

例:

```yaml
server: "connect.proxlane.com:4610"
token: "slk_xxx"
region: "jp"
log_level: "info"
```
...
| Command | 説明 |
| --- | --- |
| `proxlane auth <TOKEN>` | token を保存 |
| `proxlane http <PORT>` | HTTP tunnel 作成 |
| `proxlane tcp <PORT>` | TCP tunnel 作成 |
| `proxlane start --config proxlane.yml` | 複数 tunnel 起動 |
| `proxlane status` | 現在の session/tunnel 表示 |
| `proxlane logout` | 保存 token を削除 |
| `proxlane version` | version 表示 |

## 9. Control Plane API 仕様

### 9.1 認証方針

ログインは Auth.js の GitHub OAuth のみを使用する。Proxlane の API は、Dashboard から送られる Auth.js session cookie を `auth()` で検証して `userId` を確定する。

Proxlane 独自の signup/password login endpoint は持たない。
...
sequenceDiagram
    ...
    participant Auth as Auth.js
    participant GitHub
    participant API as Proxlane API
...
```json
{
  "id": "tok_123",
  "name": "macbook",
  "token": "generated-agent-token-shown-once",
...
Auth.js Prisma Adapter の標準モデルを基礎にし、Proxlane 独自モデルを追加する。ID は Prisma の `String @id @default(cuid())` を基本とする。

### 10.1 Auth.js models

| Model | 用途 |
| --- | --- |
| `User` | Auth.js user。Proxlane の owner |
...
| publicUrl | String | public endpoint |
| hostname | String? | HTTP hostname |
| tcpPort | Int? | TCP port |
| protectionType | String? | `basic` / `bearer`。secret は保存しない |
...
Proxlane は OSS として開発するため、公開リポジトリに漏れた時点で危険な値をコード、設定例、テストログに含めない。`.env.example` は必ずダミー値だけにする。
...
### 12.4.1 Endpoint Protection

HTTP tunnel は CLI で `--basic-auth user:pass` または `--bearer token` を指定できる。Relay は外部 request を Agent に転送する前に認証し、失敗時は `401 Unauthorized` を返す。認証成功後、protection に使った `Authorization` header はローカル service へ転送しない。

DB には `TunnelSession.protectionType` として `basic` / `bearer` の種別だけを保存する。Basic Auth password や Bearer token の値は保存しない、ログ出力しない、Dashboard に表示しない。
...
relay:
  id: "jp-1"
  public_base_domain: "t.proxlane.com"
  tcp_public_host: "jp-1.tcp.proxlane.com"
  control_addr: ":4610"
  http_addr: ":80"
  https_addr: ":443"
  tcp_port_range:
...
control_plane:
  base_url: "https://proxlane.com"
  internal_secret: "${PROXLANE_RELAY_INTERNAL_SECRET}"

tls:
  mode: "manual"
  cert_file: "/etc/proxlane/cert.pem"
  key_file: "/etc/proxlane/key.pem"
